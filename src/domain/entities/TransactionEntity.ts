...
export class TransactionEntity {
  ...
  public validateModel() {
    if (!this.model) return 'Model is required.';

    return '';
  }

  public validatePlateNumber() {
    if (!this.plate_number) return 'Plate number is required';

    if (this.plate_number.length < 6) return 'Plate number must be 6 characters long';

    return '';
  }

  public validateServices() {
    if (this.services.length === 0) return 'Services is required';
