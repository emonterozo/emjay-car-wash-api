...
export class TransactionRepository implements ITransactionRepository {
  ...
  public async findAll(params?: IGetAllTransactionsParams): Promise<TransactionObject[]> {
    ...
    const collection: Collection<ITransactionCollection> = database.collection(
      process.env.MONGO_TRANSACTIONS_COLLECTION!,
    );

    const and = params?.and_conditions?.map((condition) => {
      if (condition.field === 'id') return { _id: new ObjectId(condition.value) };

      if (condition.field === 'customer_id') return { customer_id: new ObjectId(condition.value) };
    ...
    return transactions.map<TransactionObject>((transac) => ({
      ...
      services: transac.services.map((service) => ({
        assigned_employee_id: service.assigned_employee_id.map((employee_id) =>
  ...
  public async save(params: ITransactionInput): Promise<InsertedId> {
    ...
    await this._mongo_client.connect();
    const database = this._mongo_client.db(process.env.MONGO_DATASOURCE);
    const collection: Collection<PartialField<ITransactionCollection, '_id'>> = database.collection(
      process.env.MONGO_TRANSACTIONS_COLLECTION!,
    );
